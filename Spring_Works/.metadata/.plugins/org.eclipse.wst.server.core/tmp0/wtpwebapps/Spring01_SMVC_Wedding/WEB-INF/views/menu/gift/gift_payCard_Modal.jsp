				...
				<table>
					<tbody>
						<tr>
							<td>결재금액</td>
							<td>
							<c:if test="${ payTotal != null }">
								<input type="text" name="payment"  
										onchange="getNumber(this);" 
										onkeyup="getNumber(this);" 
										style="text-align:right;" size="10" value="${payTotal}">원
								<c:forEach var="each" items="${ checkArrSender }">
									<input type="hidden" name="num" value="${ each.num }">
									<input type="hidden" name="name" value="${ each.name }">
									<input type="hidden" name="quantity" value="${ each.buyquantity }">
									<input type="hidden" name="category" value="${ each.type }">
								</c:forEach>
							</c:if>
							<c:if test="${ payTotal == null }">
								<input type="text" name="payment"  
										onchange="getNumber(this);" 
										onkeyup="getNumber(this);" 
										style="text-align:right;" size="10" value="${gcDto02.total }">원
								<input type="hidden" name="name" value="${ gcDto02.name }">
								<input type="hidden" name="quantity" value="${ gcDto02.buyquantity }">
								<input type="hidden" name="category" value="gift_${ gcDto02.type }">
								<input type="hidden" name="num" value="${ gcDto02.num }">
							</c:if>
							</td>
						</tr>
						<tr>
