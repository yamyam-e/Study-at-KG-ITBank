...
public class RegistryProductController {
	...
	public ModelAndView regSelectListCart(HttpSession session, ModelMap mmap){
	    ...
	    mmap.addAttribute("totalPoint", getTotalPoint(email));
	...
	public ModelAndView regPayCard(
		...
		if(payStyle.equals("one")){
			...
			Integer point = Integer.parseInt(request.getParameter("payment"));
			SavedPointDTO savedDTO = new SavedPointDTO(email, point, request.getParameter("cardHolder"));
			int res = repository.insertSavedPoint(savedDTO);
			mmap.addAttribute("totalPoint", getTotalPoint(email));
		}
