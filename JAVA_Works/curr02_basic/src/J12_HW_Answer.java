		...
		while(true){
			...
			if(sel.equals("1")) { // 학생 정보 입력	
				...
				if(stuName != null) {
					...
					if(ans.equalsIgnoreCase("n")) { // n 입력
						JOptionPane.showMessageDialog(null, new JTextArea("메뉴로 돌아갑니다."));
						continue; // while 조건식으로 이동 vs for 문에서는 증감식으로 이동
					} else if (!ans.equalsIgnoreCase("y")) { // 오타 입력
						JOptionPane.showMessageDialog(null, new JTextArea("잘못된 입력입니다."));
						continue; // while 조건식으로 이동 vs for 문에서는 증감식으로 이동
					}
					// y 입력 : 아래 명령 실행
				}
